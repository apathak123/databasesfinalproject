...
<h3>Bar Query 3</h3>
want to know what the most popular beers you sell are from what the world likes?<br/>
          enter in the following information!<br/>
      (use Rebel Moustache Inn)<br/>

<form name="f6" method="get" action="#">
  Bar name:<br>
  <input type="text" name="bar_name"><br>
  ...
  <% String bar_name=request.getParameter("bar_name");
     ...
     try{
    	//Get the database connection
			...
			if (bar_name!=null)
			{
			String str = "select Likes.beer_name, count(Likes.beer_name)as how_many_people_like_the_beer from Likes where exists (select * from Sells where Sells.bar_name='"+bar_name+"' and Likes.beer_name like concat('%',Sells.beer_name,'%')) group by Likes.beer_name order by count(Likes.beer_name) desc";
			...
			<thead>
			<tr><td>beer_name</td><td></td><td>how_many_people_like_the_beer</td></tr>
			</thead>
			<tbody>
		<% while (result.next()) 
		{%>
			...
			<td><%out.print(result.getString("how_many_people_like_the_beer")); %></td>
			</tr>
